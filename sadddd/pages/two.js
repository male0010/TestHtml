import Head from 'next/head'
import Image from 'next/image'
import { Inter } from 'next/font/google'
import styles from '@/styles/Home.module.css'
import imghead from '../pages/img/Bootstrap_logo.png'
import Link from 'next/link'
import Header from './head.js'
import s1 from '../pages/img/ss1.png'
import s2 from '../pages/img/ss2.png'
import s3 from '../pages/img/ss3.png'
import s4 from '../pages/img/ss4.png'
import s5 from '../pages/img/ss5.png'
import React from 'react';
import { useTranslation } from 'react-i18next';
import i18n from "i18next";
import { useEffect, useRef, useState } from 'react';

export default function Home() {
  const { t } = useTranslation();
  const handleChangeLang = (lang, event) => {
    event.preventDefault();
    i18n.changeLanguage(lang);
  }
  

  return (
    <>
      <Head>
        <title>bootstrap®</title>
        <meta name="description" content="Generated by create next app" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <main className='ff lh-base'>

        <nav className="navbar navbar-expand-lg preview__header   text-white bg-black  fixed-top ">
          <div className="container ">
            <svg baseProfile="tiny" height={20} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 304 36"><path fill="#81B441" d="M22.038 6.715c-.865-.48-3.344-.183-6.322.738-5.212 3.562-9.61 8.811-9.917 17.238-.056.202-.57-.027-.672-.09-1.407-2.697-1.965-5.538-.79-9.635.219-.365-.497-.814-.626-.685-.258.258-1.333 1.404-2.047 2.642-3.54 6.141-1.225 14.009 4.968 17.453 6.192 3.449 14.011 1.224 17.459-4.97 3.989-7.148.285-21.385-2.053-22.691z" /><g fill="#fff"><path d="M44.202 10.145c-7.342 0-12.671 5.458-12.671 12.978 0 7.582 5.315 12.877 12.927 12.877 3.845 0 6.888-1.202 9.309-3.677.723-.781.801-1.533.801-1.821 0-1.226-.912-2.117-2.168-2.117-.633 0-1.163.239-1.77.801-1.371 1.277-3.176 2.582-6.172 2.582-4.166 0-7.554-3.128-7.768-7.145h16.581c1.803 0 2.679-.843 2.679-2.576 0-.51 0-.879-.103-1.605-.893-6.447-5.246-10.297-11.645-10.297zm0 4.026c3.981 0 6.536 2.537 6.693 6.632h-14.2c.373-3.789 3.577-6.632 7.507-6.632zM71.785 10.145c-3.335 0-6.637 1.918-7.927 4.581v-1.646c0-2.55-1.976-2.679-2.372-2.679-1.171 0-2.423.704-2.423 2.679v19.881c0 2.648 2.104 2.782 2.525 2.782.421 0 2.526-.134 2.526-2.782v-10.657c0-4.637 2.628-7.876 6.39-7.876 3.644 0 5.416 2.391 5.416 7.312v11.222c0 2.648 2.105 2.782 2.526 2.782.421 0 2.525-.134 2.525-2.782v-12.81c0-4.838-2.414-10.007-9.186-10.007zM103.825 10.402c-1.167 0-2.014.681-2.518 2.02l-6.911 17.378-6.858-17.378c-.537-1.36-1.377-2.02-2.569-2.02-1.393 0-2.526 1.087-2.526 2.423 0 .331.038.759.317 1.429l7.942 18.753c.946 2.262 2.503 2.737 3.643 2.737 1.139 0 2.696-.475 3.645-2.736l7.994-18.857c.273-.662.315-1.147.315-1.377 0-1.352-1.065-2.372-2.474-2.372zM118.513 10.145c-3.231 0-6.24.9-8.472 2.532-.757.522-1.125 1.174-1.125 1.993 0 1.128.861 2.012 1.963 2.012.476 0 1.01-.19 1.499-.532 1.919-1.279 3.722-1.876 5.673-1.876 3.735 0 5.879 1.899 5.879 5.212v.549c-8.384.009-16.91 1.022-16.91 8.315 0 5.249 4.486 7.599 8.932 7.599 3.54 0 6.368-1.403 8.182-4.059v1.277c0 1.902 1.277 2.576 2.373 2.576.231 0 2.269-.072 2.269-2.576v-13.373c-.002-6.042-3.839-9.649-10.263-9.649zm4.354 13.609h1.063v1.164c0 4.256-2.71 7.006-6.903 7.006-1.138 0-4.852-.265-4.852-3.676-.001-4.029 6.101-4.494 10.692-4.494zM141.928 14.991c2.21 0 2.322-1.72 2.322-2.064 0-1.023-.61-2.116-2.322-2.116h-4.545v-4.596c0-2.053-1.332-2.782-2.474-2.782-.421 0-2.525.134-2.525 2.782v21.829c0 5.037 2.485 7.7 7.187 7.7 1.217 0 2.402-.215 3.178-.576.889-.444 1.398-1.173 1.398-2 0-1.167-.847-2.014-2.014-2.014-.225 0-.546.056-.9.158-.433.108-.726.15-1.046.15-1.913 0-2.804-1.249-2.804-3.931v-12.54h4.545zM158.631 10.145c-7.64 0-13.183 5.415-13.183 12.875 0 3.632 1.343 6.942 3.78 9.321 2.419 2.359 5.758 3.659 9.402 3.659 7.544 0 13.234-5.58 13.234-12.979.001-7.461-5.566-12.876-13.233-12.876zm0 21.573c-5.239 0-7.979-4.375-7.979-8.697 0-5.939 4.136-8.644 7.979-8.644s7.979 2.705 7.979 8.644c0 5.975-4.136 8.697-7.979 8.697z" /></g><g fill="#81B441"><path d="M211.554 11.937c-.739-.805-1.655-1.403-2.725-1.777-1.951-.68-4.619-.735-6.739-.061-.994.316-1.909.754-2.722 1.302-.815.552-1.53 1.207-2.126 1.947-.187.231-.363.47-.527.715-.52-1.284-1.285-2.283-2.288-2.978-1.401-.972-3.205-1.465-5.363-1.465-1.102 0-2.146.176-3.105.522-.947.342-1.823.815-2.604 1.404-.767.579-1.448 1.273-2.023 2.061v-2.787c0-.418-.339-.757-.757-.757h-1.219c-.418 0-.757.339-.757.757v23.98c0 .418.339.757.757.757h1.219c.418 0 .757-.339.757-.757v-15.087c.14-.709.395-1.502.759-2.359.368-.865.874-1.691 1.504-2.451.617-.744 1.387-1.373 2.288-1.871.872-.481 1.92-.725 3.114-.725.873 0 1.69.102 2.426.305.689.19 1.266.511 1.763.982.493.469.886 1.122 1.169 1.94.296.854.446 1.967.446 3.309v15.957c0 .418.339.757.757.757h1.197c.418 0 .757-.339.757-.757l-.002-15.906c.025-.361.117-.778.272-1.238.16-.473.387-.957.675-1.44.292-.49.654-.975 1.077-1.445.41-.457.902-.872 1.464-1.233.559-.36 1.203-.655 1.911-.879.705-.221 1.502-.339 2.368-.353l.155-.001c.806 0 1.562.101 2.247.303.688.202 1.264.541 1.761 1.04.496.495.892 1.196 1.178 2.08.299.925.451 2.121.451 3.557v15.515c0 .418.339.757.757.757h1.197c.418 0 .757-.339.757-.757v-15.515c0-1.664-.187-3.111-.555-4.302-.377-1.224-.94-2.249-1.671-3.046zM235.82 33.56c-.07-.337-.129-.697-.178-1.083-.05-.392-.089-.804-.117-1.239-.029-.434-.043-.857-.043-1.27v-12.012c0-1.355-.234-2.573-.697-3.62-.467-1.055-1.129-1.945-1.969-2.646-.828-.689-1.823-1.215-2.957-1.561-2.266-.689-5.181-.69-7.45.098-1.161.401-2.186.958-3.045 1.656-.869.708-1.554 1.542-2.035 2.481-.489.953-.737 1.968-.737 3.015 0 .407.322.741.729.756l1.197.044c.202.016.404-.069.553-.211.148-.142.232-.339.232-.545 0-.705.161-1.344.494-1.953.339-.62.814-1.166 1.413-1.623.615-.47 1.354-.843 2.197-1.111.852-.27 1.797-.407 2.807-.407 2.149 0 3.805.496 4.92 1.474 1.101.967 1.637 2.31 1.637 4.108v2.745h-4.984c-1.783 0-3.413.191-4.847.568-1.45.383-2.701.925-3.721 1.612-1.049.708-1.863 1.575-2.421 2.575-.564 1.011-.85 2.127-.85 3.316 0 1.061.197 2.05.584 2.939.389.895.953 1.676 1.674 2.323.716.642 1.585 1.143 2.583 1.491.986.344 2.095.519 3.297.519 1.255 0 2.417-.139 3.451-.414 1.031-.272 1.979-.651 2.817-1.125.839-.475 1.585-1.033 2.22-1.66l.275-.281.024.315c.071.837.195 1.568.371 2.175.093.324.39.547.727.547h1.374c.418 0 .757-.339.757-.757v-.199c0-.104-.022-.208-.064-.303-.081-.187-.155-.435-.218-.737zm-11.734-.248c-.825 0-1.58-.122-2.247-.365-.66-.239-1.235-.571-1.708-.987-.466-.409-.833-.891-1.091-1.43-.253-.531-.38-1.107-.38-1.714 0-.768.189-1.455.578-2.102.396-.658.978-1.234 1.731-1.715.783-.499 1.758-.895 2.9-1.177 1.163-.287 2.503-.433 3.985-.433h4.917v4.774c-.357.761-.799 1.452-1.315 2.058-.546.641-1.189 1.198-1.913 1.658-.722.459-1.547.816-2.452 1.061-.913.246-1.924.372-3.005.372zM250.599 9.913c-.244-.094-.527-.166-.863-.218-.319-.051-.687-.076-1.093-.076-2.123 0-3.924.544-5.352 1.615-.644.482-1.216 1.046-1.715 1.688v-2.102c0-.418-.339-.757-.757-.757h-1.197c-.418 0-.757.339-.757.757v23.98c0 .418.339.757.757.757h1.197c.418 0 .757-.339.757-.757v-15.646c.18-.986.466-1.908.851-2.744.383-.833.875-1.564 1.463-2.172.579-.598 1.273-1.074 2.062-1.416.787-.341 1.692-.514 2.691-.514.251 0 .492.013.723.039l.735.088c.198.024.4-.031.558-.153.158-.123.261-.304.287-.503l.133-1.064c.042-.347-.155-.677-.48-.802zM260.642 21.099l10.484-9.725c.228-.212.303-.541.19-.832-.114-.289-.393-.48-.704-.48h-1.707c-.19 0-.374.072-.513.202l-10.911 10.09-1.538 1.454v-21.051c0-.418-.339-.757-.757-.757h-1.197c-.418 0-.757.339-.757.757v34.043c0 .418.339.757.757.757h1.197c.418 0 .757-.339.757-.757v-9.582l2.687-2.31 10.541 12.383c.144.169.355.266.576.266h1.573c.418 0 .757-.339.757-.757 0-.244-.116-.461-.295-.599l-11.14-13.102zM288.498 12.708c-.873-.976-1.929-1.745-3.139-2.281-2.474-1.098-5.643-1.093-8.216.154-1.307.633-2.462 1.528-3.435 2.66-.964 1.121-1.738 2.475-2.299 4.024-.559 1.547-.843 3.256-.843 5.079v.975c0 1.827.292 3.531.869 5.064.577 1.539 1.384 2.888 2.396 4.008 1.016 1.127 2.22 2.019 3.578 2.651 1.366.635 2.853.957 4.42.957 1.015 0 1.963-.104 2.815-.309.853-.206 1.645-.495 2.353-.861.713-.369 1.361-.818 1.925-1.333.557-.509 1.056-1.081 1.486-1.701.235-.341.154-.807-.184-1.047l-.842-.598c-.33-.236-.788-.169-1.036.154-.462.597-.945 1.091-1.437 1.47-.491.379-1.009.686-1.54.911-.532.227-1.096.388-1.675.481-1.725.278-3.632.16-5.208-.618-1.041-.514-1.957-1.223-2.72-2.109-.77-.893-1.384-1.961-1.823-3.177-.441-1.219-.664-2.547-.664-3.943v-.551h17.218c.418 0 .757-.339.757-.757v-1.396c0-1.602-.243-3.09-.721-4.422-.485-1.342-1.169-2.514-2.035-3.485zm-4.19.263c.876.432 1.632 1.024 2.248 1.757.625.747 1.117 1.63 1.461 2.623.298.858.47 1.767.512 2.707h-15.079c.155-.911.406-1.775.75-2.58.444-1.037 1.027-1.954 1.734-2.722.699-.759 1.518-1.36 2.435-1.789 1.795-.838 4.188-.855 5.939.004zM303.675 33.715c-.071-.401-.446-.661-.852-.617-.403.058-.778.106-1.124.141-.654.069-1.575.107-2.316-.108-.303-.087-.566-.255-.806-.518-.251-.272-.459-.683-.617-1.219-.17-.577-.257-1.363-.257-2.335v-16.264h4.696c.418 0 .757-.339.757-.757v-1.219c0-.418-.339-.757-.757-.757h-4.696v-5.715c0-.418-.339-.757-.757-.757h-1.219c-.418 0-.757.339-.757.757v5.715h-3.676c-.418 0-.757.339-.757.757v1.219c0 .418.339.757.757.757h3.676v16.265c0 1.26.121 2.335.36 3.195.252.907.631 1.649 1.128 2.207.511.573 1.142.983 1.877 1.215.677.215 1.448.324 2.29.324l.583-.012c.234-.008.475-.024.732-.047l.713-.08c.275-.037.522-.095.735-.172.345-.126.55-.481.487-.843l-.2-1.132z" /></g>
              <a className="navbar-brand text-white" href="#"> </a>
            </svg>
            <div className="" id="navbarSupportedContent">
              <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              </ul>
              <form className="d-flex justify-content-end my-1" role="search">

                <button className="btn btn-success fw-bold" type="submit">buy now</button>
              </form>
            </div>
          </div>
        </nav>


        <nav className="navbar navbar-expand-lg theme-main-menu sticky-menu theme-menu-four bg-light pp p-2 fixed-top  ">
          <div className="container">
            <a className="navbar-brand" href="#">
              <svg xmlns="http://www.w3.org/2000/svg" height="30" viewBox="5.749 89.122 1149.031 231.404" id="bootstrap"><path fill="#573E7D" d="M5.752 89.561c.944-.173 1.718-.441 2.489-.439 31.816.123 63.634.076 95.446.52 10.842.15 20.804 3.975 29.711 10.315 20.331 14.467 22.209 55.797-5.701 70.622-1.42.754-2.834 1.516-4.762 2.546 5.304 2.435 10.522 4.067 14.896 6.972 16.787 11.152 23.624 36.104 16.109 58.02-6.558 19.121-22.128 27.017-40.235 31.217-5.947 1.38-12.175 2.096-18.283 2.139-29.741.215-59.484.096-89.673.096.003-60.6.003-121.208.003-182.008zm31.809 155.334c22.12 0 43.602.504 65.044-.182 13.479-.431 23.219-14.879 22.408-28.932-.799-13.852-9.329-25.632-22.514-26.248-21.257-.993-42.572-.712-63.864-.934-.318-.002-.641.466-1.075.803l.001 55.493zm.074-80.009c19.771 0 38.838.72 57.82-.239 13.979-.707 21.869-11.884 21.346-27.163-.463-13.528-8.451-21.664-22.832-22.044-18.031-.473-36.082-.254-54.126-.296-.734-.002-1.469.394-2.208.604v49.138zM1023.55 139.882h26.521v14.214c2.953-2.277 6.549-4.806 9.867-7.655 20.32-17.44 68.52-15.3 85.016 19.821 14.195 30.226 13.578 60.623-5.584 88.763-10.613 15.586-27.047 21.156-45.662 20.154-16.174-.873-30.414-5.957-41.619-20.931v66.277h-28.537c-.002-60.268-.002-120.284-.002-180.643zm26.942 66.556c1.195 6.619 1.852 13.397 3.699 19.83 4.123 14.356 16.678 24.972 29.682 25.889 17.395 1.227 29.887-5.2 36.174-20.622 7.992-19.599 7.709-39.365-3.971-57.682-13.775-21.607-46.787-19.801-58.461 2.834-4.805 9.318-7.157 19.125-7.123 29.751zM1004.761 252.715c0 5.417.191 10.918-.139 16.391-.08 1.351-1.775 3.633-2.873 3.727-8.271.689-16.678 1.895-24.844 1.043-7.199-.752-11.166-6.854-13.385-13.91-17.09 13.913-36.688 16.922-57.535 14.339-17.434-2.159-31.875-16.474-33.117-32.368-1.715-21.906 6.701-36.17 26.371-41.363 13.795-3.64 28.164-5.103 42.271-7.562 3.422-.596 6.885-1.082 10.227-1.996 6.42-1.752 11.051-5.306 11.521-12.614.486-7.554-2.443-13.44-9.543-16.299-12.117-4.875-24.453-4.754-36.383.828-5.443 2.546-8.676 7.67-11.527 16.373H877.85c.023-10.657 3.732-19.614 10.811-27.155 9.352-9.962 21.539-13.873 34.666-15.246 13.74-1.437 27.402-1.373 40.75 3.128 17.951 6.053 26.547 17.896 26.521 36.856-.025 20.126-.008 40.252-.012 60.381-.004 14.284-.006 14.284 14.175 15.447zm-43.205-45.416c-8.811 1.526-16.871 2.696-24.826 4.36-8.367 1.75-16.895 3.266-24.873 6.188-7.822 2.866-10.48 10.176-10.221 18.026.246 7.505 5.373 11.71 11.98 14.019 11.332 3.959 22.691 3.471 33.5-1.584 7.74-3.621 13.633-9.541 14.326-18.586.571-7.534.114-15.147.114-22.423zM242.249 136.079c43.841-.515 69.55 31.621 66.838 75.378-.941 15.209-5.028 29.423-15.039 41.443-10.788 12.951-24.669 19.771-41.31 21.621-13.896 1.549-27.399.639-40.275-5.205-19.175-8.705-30.113-23.896-34.714-44.137-3.155-13.873-3.017-27.688.52-41.411 7.803-30.272 31.114-47.571 63.98-47.689zm-38.163 69.828c1.151 6.423 1.752 13.006 3.559 19.239 4.599 15.869 17.124 26.347 31.444 27.041 17.701.858 30.086-6.588 36.664-23.076 6.423-16.096 6.433-32.569-.609-48.471-10.433-23.559-38.857-29.583-57.16-12.072-10.437 9.984-13.131 23.251-13.898 37.339zM394.354 136.081c39.542-.062 66.447 26.584 66.947 66.217.212 16.735-2.514 32.693-12.338 46.712-14.062 20.07-34.374 27.092-57.759 26.027-8.616-.393-17.548-2.307-25.578-5.459-18.854-7.401-30.096-21.973-35.092-41.477-4.385-17.127-3.897-34.146 1.681-50.886 8.555-25.678 32.023-41.089 62.139-41.134zm37.901 66.267c-.638-3.515-1.267-10.307-3.162-16.727-4.817-16.328-17.548-26.241-33.588-26.938-15.106-.658-28.693 8.249-34.552 23.848-6.901 18.373-6.393 36.674 4.005 53.793 10.832 17.834 35.387 21.742 51.491 8.436 12.287-10.153 15.293-24.171 15.806-42.412zM676.736 176.492c-9.023 0-17.811.074-26.592-.094-1.082-.02-2.766-1.104-3.115-2.075-4.938-13.716-16.557-15.347-28.572-15.416-4.982-.03-10.283 1.183-14.896 3.118-9.859 4.136-10.955 15.291-1.715 20.634 6.41 3.707 13.936 5.698 21.166 7.734 10.23 2.881 20.791 4.616 30.955 7.687 25.988 7.855 36.145 33.911 21.594 55.391-6.98 10.301-17.418 15.877-29.387 18.766-17.689 4.269-35.301 4.582-52.588-1.661-18.497-6.683-28.268-20.095-29.71-40.396H592.1c8.025 20.947 30.701 29.105 51.945 18.401 4.914-2.478 8.195-6.001 8.523-11.793.35-6.19-2.041-11.513-7.596-13.585-9.904-3.694-20.248-6.227-30.416-9.208-8.422-2.47-17.111-4.254-25.238-7.47-15.678-6.209-22.49-16.891-22.394-33.176.084-14.204 8.387-26.374 23.109-31.712 20.262-7.347 40.941-8.261 61.293-.288 13.76 5.39 21.961 16.02 25.371 30.332.308 1.321.039 2.785.039 4.811zM523.501 139.515h26.123v21.361h-25.598c-.181 2.558-.414 4.348-.417 6.137-.025 21.561-.035 43.121.01 64.678.005 2.869.227 5.746.51 8.604.562 5.701 3.727 9.087 9.467 9.399 5.26.289 10.549.062 16.119.062v21.359c-11.931 2.953-24.057 2.797-35.796.182-13.669-3.044-19.222-12.449-19.342-30.251-.165-24.228-.049-48.46-.054-72.69v-7.123h-21.696v-21.605h21.403v-39.562h29.274c-.003 12.941-.003 25.787-.003 39.449zM767.81 271.938c-11.24 0-21.881.35-32.486-.1-10.279-.436-18.195-7.253-20.441-17.427a86.38 86.38 0 0 1-2.023-17.662c-.227-22.795-.094-45.596-.094-68.394v-7.209h-21.625v-21.319h21.303v-39.723h29.33v39.079h25.998v21.474h-24.947c-.297 1.935-.686 3.273-.686 4.614-.021 23.209-.096 46.418.055 69.625.08 12.274 3.293 15.228 15.459 15.141 3.25-.021 6.504-.002 10.158-.002l-.001 21.903zM816.597 139.956v22.475c11.264-19.123 26.852-29.088 49.873-25.691v27.727c-10.936-1.644-21.484-1.937-30.826 4.776-12.254 8.809-16.533 21.681-16.838 35.87-.471 21.935-.123 43.89-.123 66.316h-28.727V139.954c8.702.002 17.221.002 26.641.002z"></path></svg>
            </a>
            <button className="navbar-toggler " type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasNavbar" aria-controls="offcanvasNavbar">
              <span className="navbar-toggler-icon"></span>
            </button>
            <div className="offcanvas offcanvas-end " tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
              <div className="offcanvas-header">
                <svg xmlns="http://www.w3.org/2000/svg" height="70" className="offcanvas-title" id="offcanvasNavbarLabel" enable-background="new 0 0 24 24" viewBox="0 0 24 24" ><path fill="#543B79" d="M21.375,0.75H2.625C1.59,0.75,0.75,1.589,0.75,2.625v18.75c0,1.036,0.84,1.875,1.875,1.875h18.75c1.035,0,1.875-0.839,1.875-1.875V2.625C23.25,1.589,22.41,0.75,21.375,0.75z"></path><path fill="#FFF" d="M17.508,16.639v0.002c-0.2,0.482-0.517,0.907-0.923,1.234c-0.42,0.34-0.952,0.62-1.607,0.82C14.324,18.898,13.546,19,12.645,19H6.213V5h6.802c1.258,0,2.266,0.283,3.02,0.86c0.76,0.58,1.138,1.444,1.138,2.61c0,0.705-0.172,1.31-0.518,1.81c-0.344,0.497-0.84,0.886-1.48,1.156v0.046c0.854,0.18,1.515,0.585,1.95,1.215c0.435,0.63,0.658,1.426,0.658,2.387c0,0.538-0.104,1.05-0.3,1.528L17.508,16.639z"></path><path fill="#543B79" d="M14.708 12.877c-.41-.375-.986-.558-1.73-.558H8.656v4.368h4.334c.74 0 1.32-.192 1.73-.58.41-.385.62-.934.62-1.64-.007-.69-.21-1.224-.62-1.59H14.708zM13.934 10.185h.004c.396-.336.59-.817.59-1.444 0-.704-.175-1.204-.53-1.49-.352-.285-.86-.433-1.528-.433h-4v3.863h4C13.053 10.682 13.55 10.511 13.934 10.185z"></path><path d="M21.375,24H2.625C1.178,24,0,22.822,0,21.375V2.625C0,1.178,1.178,0,2.625,0h18.75C22.822,0,24,1.178,24,2.625v18.75C24,22.822,22.822,24,21.375,24z M2.625,1.5C2.005,1.5,1.5,2.005,1.5,2.625v18.75c0,0.62,0.505,1.125,1.125,1.125h18.75c0.62,0,1.125-0.505,1.125-1.125V2.625c0-0.62-0.505-1.125-1.125-1.125H2.625z"></path><path d="M12.644,19.5H6.241c-0.414,0-0.75-0.336-0.75-0.75V5.25c0-0.414,0.336-0.75,0.75-0.75h6.771c1.416,0,2.577,0.329,3.452,0.977c0.953,0.706,1.437,1.755,1.437,3.12c0,0.837-0.221,1.571-0.655,2.181c-0.143,0.2-0.306,0.385-0.487,0.554c0.377,0.24,0.696,0.543,0.955,0.906c0.527,0.741,0.795,1.661,0.795,2.737c0,0.542-0.095,1.071-0.282,1.576c-0.006,0.075-0.022,0.148-0.053,0.218c-0.248,0.582-0.645,1.097-1.147,1.489c-0.504,0.396-1.126,0.706-1.848,0.919C14.464,19.391,13.61,19.5,12.644,19.5z M6.991,18h5.652c0.819,0,1.528-0.088,2.106-0.262c0.542-0.16,0.996-0.383,1.353-0.663c0.256-0.2,0.466-0.451,0.615-0.733c0.009-0.063,0.025-0.126,0.052-0.188c0.159-0.375,0.239-0.772,0.239-1.181c0-0.757-0.174-1.385-0.517-1.866c-0.315-0.443-0.813-0.736-1.479-0.872c-0.35-0.071-0.601-0.378-0.601-0.735c0-0.305,0.185-0.624,0.467-0.739c0.501-0.205,0.887-0.493,1.146-0.856c0.253-0.354,0.376-0.783,0.376-1.309c0-0.887-0.264-1.496-0.829-1.914C14.961,6.229,14.101,6,13.013,6H6.991V18z"></path><path d="M12.966 16.977H8.745c-.414 0-.75-.336-.75-.75V12.78c0-.414.336-.75.75-.75h4.209c.852 0 1.521.172 2.044.525.032.017.063.036.094.058.588.426.903 1.066.913 1.853 0 .797-.311 1.451-.898 1.9C14.577 16.771 13.857 16.977 12.966 16.977zM9.495 15.477h3.471c.554 0 .967-.102 1.229-.303.145-.111.31-.289.31-.699-.004-.338-.122-.505-.241-.606-.024-.014-.048-.029-.07-.046-.263-.194-.68-.293-1.239-.293H9.495V15.477zM12.382 11H8.745c-.414 0-.75-.336-.75-.75v-3c0-.414.336-.75.75-.75h3.637c.755 0 1.366.158 1.816.47.366.252.805.766.805 1.774 0 .73-.293 1.328-.847 1.729-.019.013-.037.026-.056.037C13.633 10.831 13.04 11 12.382 11zM9.495 9.5h2.887c.37 0 .669-.082.89-.242.013-.009.025-.018.038-.026.104-.084.193-.209.193-.489 0-.161-.021-.445-.157-.54C13.211 8.11 12.935 8 12.382 8H9.495V9.5z"></path></svg>
                <button type="button" className="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
              </div>
              <div className="offcanvas-body">
                <ul className="navbar-nav justify-content-center   flex-grow-1  ">
                  <li className="nav-item dropdown ">
                    <a className="nav-link dropdown-toggle fw-bold" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                      Home
                    </a>

                    <ul className="dropdown-menu">
                      <li><a className="dropdown-item" href="#">Action</a></li>
                      <li><a className="dropdown-item" href="#">Another action</a></li>
                      <li>
                        <hr className="dropdown-divider" />
                      </li>
                      <li><a className="dropdown-item" href="#">Something else here</a></li>
                    </ul>
                  </li>
                  <li className="nav-item dropdown">
                    <a className="nav-link dropdown-toggle fw-bold" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                      Pages
                    </a>
                    <ul className="dropdown-menu">
                      <li><a className="dropdown-item" href="#">Action</a></li>
                      <li><a className="dropdown-item" href="#">Another action</a></li>
                      <li>
                        <hr className="dropdown-divider" />
                      </li>
                      <li><a className="dropdown-item" href="#">Something else here</a></li>
                    </ul>
                  </li>
                  <li className="nav-item dropdown">
                    <a className="nav-link dropdown-toggle fw-bold" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                      Blogs
                    </a>
                    <ul className="dropdown-menu">
                      <li><a className="dropdown-item" href="#">Action</a></li>
                      <li><a className="dropdown-item" href="#">Another action</a></li>
                      <li>
                        <hr className="dropdown-divider" />
                      </li>
                      <li><a className="dropdown-item" href="#">Something else here</a></li>
                    </ul>
                  </li>
                </ul>
                <form className="d-flex " >
                  <botton className="theme-btn-five my-2 py-3    px-5 me-3"
                    onClick={(event) => handleChangeLang('en', event)}
                  >Eng</botton>

                  <botton className="theme-btn-five my-2 py-3 px-5" onClick={(event) => handleChangeLang('th', event)}
                  >Thai</botton>
                </form>
              </div>
            </div>
          </div>
        </nav>
        <div className="hero-banner-four  ">
          <div className="container px-4">
            <div className="row ">
              <div className="col-lg-4 order-lg-last">
                <div className="illustration-holder p-2 fs-4 text-center m-2   d-flex justify-content-center flex-column h-100">
                  <svg xmlns="http://www.w3.org/2000/svg" className="illustration card-img fade-in-image " height='321' enable-background="new 0 0 64 64" viewBox="0 0 64 64" id="birds-house"><path d="M40.71 29.61c-.13 0-.26-.03-.38-.1-.4-.21-.56-.71-.35-1.12l2.03-3.87c.21-.4.71-.56 1.12-.35.4.21.56.71.35 1.12l-2.03 3.87C41.3 29.45 41.01 29.61 40.71 29.61zM36.37 29.61c-.19 0-.39-.07-.55-.21-.34-.3-.38-.83-.07-1.17l4.33-4.92c.3-.34.83-.38 1.17-.07.34.3.38.83.07 1.17l-4.33 4.92C36.83 29.51 36.6 29.61 36.37 29.61z"></path><path d="M61.35 27c-.34 0-.68-.08-1-.24l-8.75-4.39-6.12 1.7c-.2.26-.53.65-.98 1-.71.56-1.62.78-2.5.61-.85-.16-1.59-.68-2.02-1.42-.05-.09-.1-.18-.14-.27-.65-.1-1.78-.35-3-.9-2.29-1.03-5.04-3.19-5.1-7.65-.01-.56.03-1.15.11-1.75.11-.85-.06-1.69-.49-2.36-.08-.12-.16-.23-.26-.34-.34-.39-.78-.72-1.31-.97-.24-.11-.41-.33-.46-.59-.05-.26.02-.52.2-.72.09-.1.9-.93 2.77-1.1.62-.4 2.46-1.42 4.6-.92 1.74.4 3.2 1.69 4.35 3.82 1.05.33 4.18 1.47 6.95 3.91.65.57 1.23 1.18 1.75 1.82l3.17 2.02 9.53 3.62c.64.24 1.13.76 1.33 1.41s.11 1.35-.27 1.92l-.53.79C62.78 26.65 62.08 27 61.35 27zM51.69 20.66c.13 0 .25.03.37.09l9.03 4.53c.26.13.58.05.74-.19l.53-.79c.13-.2.1-.4.07-.5-.03-.1-.12-.28-.34-.36l-9.57-3.63c-.05-.02-.14-.06-.19-.09l-3.36-2.14c-.08-.05-.15-.11-.2-.18-.48-.6-1.03-1.18-1.64-1.72-3.03-2.68-6.6-3.65-6.64-3.66-.23-.06-.42-.22-.53-.43-.94-1.86-2.09-2.97-3.42-3.27-1.8-.42-3.43.77-3.45.78-.13.1-.28.16-.45.17-.32.02-.6.07-.83.12.19.17.38.35.54.54.14.17.28.34.39.53.64 1 .91 2.23.74 3.48-.07.52-.1 1.03-.1 1.51.04 2.88 1.42 4.95 4.12 6.16 1.56.7 2.99.83 3.01.83.36.03.66.29.74.65.03.13.07.25.14.36.19.33.51.55.89.62.41.08.84-.03 1.17-.29.49-.38.79-.86.8-.86.11-.17.28-.3.48-.35l6.72-1.87C51.55 20.67 51.62 20.66 51.69 20.66zM4.26 29.61c-.46 0-.83-.37-.83-.83V14.88c0-.46.37-.83.83-.83s.83.37.83.83v13.89C5.09 29.24 4.72 29.61 4.26 29.61zM23.43 29.61c-.46 0-.83-.37-.83-.83V14.88c0-.46.37-.83.83-.83.46 0 .83.37.83.83v13.89C24.26 29.24 23.89 29.61 23.43 29.61z"></path><path d="M26.46 15.71H.95c-.33 0-.64-.2-.76-.51-.13-.31-.06-.66.18-.9l12.9-13.05c.16-.16.33-.23.59-.25.22 0 .44.09.59.25L27.06 14.3c.23.24.3.59.17.9C27.1 15.51 26.8 15.71 26.46 15.71zM2.93 14.05h21.58L13.85 3.02 2.93 14.05zM13.85 25.43c-1.79 0-3.24-1.45-3.24-3.24 0-1.79 1.45-3.24 3.24-3.24 1.79 0 3.24 1.45 3.24 3.24C17.09 23.97 15.64 25.43 13.85 25.43zM13.85 20.6c-.87 0-1.58.71-1.58 1.58 0 .87.71 1.58 1.58 1.58.87 0 1.58-.71 1.58-1.58C15.44 21.31 14.72 20.6 13.85 20.6zM51.92 29.61H2.57c-.46 0-.83-.37-.83-.83s.37-.83.83-.83h49.35c.46 0 .83.37.83.83S52.38 29.61 51.92 29.61z"></path><path d="M13.7,63.08c-0.46,0-0.83-0.37-0.83-0.83V28.78c0-0.46,0.37-0.83,0.83-0.83c0.46,0,0.83,0.37,0.83,0.83v33.47
			            C14.53,62.71,14.16,63.08,13.7,63.08z"></path></svg>
                  <div classname=' fly'>
                    <svg xmlns="http://www.w3.org/2000/svg" className="shapes fly  shape-one " height='321' enable-background="new 0 0 64 64" viewBox="0 0 64 64" id="bird"><path d="M62.08,26.98l-5.58-1.42c-2.28-4.02-5.48-3.58-7.27-2.89c-1.37,0.52-2.58,1.52-3.48,2.88c-1.77,2.66-3.21,3.47-3.82,3.71
		c-0.34-0.3-0.92-0.77-1.73-1.32c1.31-8.49-5.91-15.42-7.4-16.75c-0.78-0.69-1.51-1.51-2.15-2.42c-2.25-3.21-5.17-8.26-5.2-8.31
		c-0.19-0.33-0.57-0.51-0.95-0.46c-0.38,0.06-0.69,0.34-0.77,0.72c-0.06,0.25-1.05,4.81,0.5,9.52c-0.58-0.53-1.01-0.96-1.16-1.19
		c-0.07-0.33-0.3-0.63-0.63-0.73c-0.42-0.13-0.88,0.05-1.1,0.43c-0.21,0.36-1.07,5.09,1.44,9.1c-0.83-0.24-1.56-0.53-1.89-0.83
		c-0.1-0.18-0.15-0.28-0.16-0.3c-0.2-0.41-0.66-0.62-1.1-0.49c-0.44,0.13-0.72,0.55-0.67,1c0.04,0.32,0.17,0.62,0.4,0.89
		c0.41,0.74,1.19,2.01,2.24,3.2c-0.03-0.01-0.07-0.01-0.1-0.02c-0.81-0.13-1.53-0.3-2.19-0.52c-2.97-1.01-10.5-4-16.18-10.02
		c-0.23-0.24-0.56-0.34-0.89-0.27c-0.32,0.07-0.58,0.31-0.68,0.63c-0.02,0.07-1.84,6.1,2.8,11.66l-1.67-0.54
		c-0.28-0.09-0.59-0.04-0.83,0.13c-0.24,0.17-0.39,0.45-0.39,0.74c-0.09,4.94,3.99,8.16,7,9.87c-1.43,0.18-2.32,0.34-2.38,0.35
		c-0.25,0.04-0.46,0.18-0.6,0.39c-0.14,0.21-0.2,0.46-0.15,0.7c0.38,2.01,1.32,3.47,2.79,4.34c1.14,0.68,2.43,0.88,3.57,0.88
		c-0.56,0.5-0.99,0.85-1,0.86c-0.24,0.2-0.37,0.52-0.33,0.83c0.04,0.32,0.24,0.59,0.52,0.73c4,1.93,8.06,1.77,11.07,1.16
		c-2.83,2.08-6.76,4.4-9.94,4.04c-0.72-0.08-1.39,0-2.05,0.09l-0.17,0.02c-1.29,0.16-2.34,0.31-3.1,0.43
		c-1.53,0.26-2.74,1.45-2.94,2.91c-0.12,0.85,0.12,1.59,0.67,2.14c0.91,0.91,2.58,1.23,5.19,0.99c-0.91,1.14-1.27,2.22-1.4,2.7
		c-0.17,0.65-0.01,1.34,0.42,1.82c0.5,0.57,1.19,0.87,1.97,0.89c1.28,0.01,2.77-0.81,3.98-1.7l-0.79,2.74
		c-0.25,0.88-0.19,1.76,0.19,2.48c0.42,0.8,1.1,1.23,1.93,1.23c0.01,0,0.01,0,0.02,0c1.7-0.02,3.6-1.97,4.51-3.81
		c0.13-0.26,0.28-0.51,0.43-0.76c0.3-0.5,0.6-1.02,0.81-1.63c1.59-4.79,4.38-7.19,5.2-7.81c0.23-0.17,0.51-0.24,0.78-0.18
		c3.42,0.78,15,2.63,20.47-6.22c0.4-0.51,3.27-4.3,2.84-8.5c0.69-0.11,1.66-0.36,2.69-0.93c1.06-0.58,1.9-1.46,2.51-2.49l6.06-2.95
		c0.35-0.17,0.56-0.55,0.52-0.93C62.74,27.4,62.46,27.08,62.08,26.98z M25.12,20.27c0.4,0.06,0.81-0.15,0.98-0.53
		c0.18-0.37,0.08-0.82-0.22-1.09c-2.43-2.18-2.94-5.14-2.97-7.09c0.82,0.77,2.03,1.8,3.83,3.22c0.37,0.29,0.91,0.26,1.24-0.07
		c0.33-0.34,0.36-0.87,0.07-1.24c-2.5-3.13-2.86-7.08-2.77-9.65c1.07,1.77,2.55,4.17,3.86,6.03c0.73,1.03,1.55,1.96,2.44,2.74
		c0.81,0.72,7.47,6.9,6.89,14.26c-2.71-1.58-6.96-3.48-13.1-4.81c-0.99-0.5-1.89-1.37-2.61-2.25
		C23.81,20.07,24.78,20.22,25.12,20.27z M52.79,32.54c-1.03,0.57-2,0.73-2.48,0.77c-0.37,0.03-0.71,0.22-0.94,0.51
		c-0.23,0.3-0.33,0.69-0.27,1.07c0.62,3.78-2.39,7.55-2.42,7.58c-0.03,0.03-0.05,0.07-0.07,0.1c-4.81,7.84-15.37,6.13-18.5,5.42
		c-0.81-0.18-1.66,0-2.33,0.51c-1.19,0.9-4.13,3.55-5.84,8.71c-0.14,0.42-0.38,0.83-0.64,1.26c-0.17,0.29-0.34,0.58-0.49,0.88
		c-0.77,1.53-2.19,2.77-2.86,2.78c-0.07,0-0.15,0-0.28-0.23c-0.15-0.29-0.17-0.68-0.05-1.09l1.68-5.83
		c0.12-0.41-0.06-0.86-0.43-1.07c-0.14-0.08-0.3-0.12-0.46-0.12c-0.25,0-0.5,0.1-0.68,0.29c-1.21,1.28-3.63,3.31-5.04,3.31
		c-0.01,0-0.01,0-0.02,0c-0.26,0-0.44-0.08-0.6-0.26c-0.01-0.01-0.02-0.06-0.01-0.11c0.17-0.67,0.99-2.95,4.12-4.17
		c0.46-0.18,0.71-0.7,0.56-1.17c-0.18-0.54-0.25-0.74-1.23-0.55l-0.01-0.02c-0.03,0.01-0.06,0.02-0.08,0.03
		c-0.57,0.12-1.43,0.35-2.75,0.72c-3.71,0.52-4.68-0.11-4.89-0.32c-0.09-0.09-0.19-0.24-0.14-0.56c0.09-0.66,0.67-1.2,1.4-1.33
		c0.73-0.12,1.77-0.27,3.02-0.42l0.18-0.02c0.57-0.07,1.11-0.14,1.6-0.08c6.49,0.73,14.81-7.3,15.16-7.64
		c0.33-0.32,0.38-0.84,0.11-1.21c-0.27-0.37-0.77-0.5-1.18-0.29c-0.07,0.03-6.45,3.22-12.84,0.97c0.67-0.59,1.5-1.35,2.1-2
		c0.29-0.31,0.33-0.78,0.1-1.14c-0.23-0.36-0.67-0.51-1.08-0.39c-0.03,0.01-3.04,0.96-5.1-0.26c-0.77-0.46-1.32-1.16-1.66-2.14
		c0.99-0.15,2.63-0.36,4.58-0.52c0.44-0.04,0.79-0.37,0.85-0.81c0.06-0.44-0.2-0.85-0.62-1c-0.08-0.03-7.77-2.79-8.78-8.23
		l4.43,1.43c0.44,0.14,0.92-0.06,1.12-0.47C9.22,25,9.1,24.5,8.72,24.24c-5.37-3.81-5.84-8.46-5.64-10.9
		c5.77,5.47,12.77,8.24,15.65,9.22c0.76,0.26,1.58,0.45,2.5,0.6C35.37,25.35,40.95,30.9,41,30.95c0.19,0.19,0.46,0.3,0.72,0.28
		c0.26-0.01,2.67-0.26,5.59-4.64c0.69-1.04,1.59-1.79,2.6-2.18c2.35-0.9,4.12,0.05,5.42,2.94C55.58,29.47,54.56,31.55,52.79,32.54z
		 M57.12,29.15c0.08-0.48,0.09-0.98,0.07-1.49l1.96,0.5L57.12,29.15z"></path><path d="M51.86,26.13c-0.52,0-0.94,0.42-0.94,0.94c0,0.52,0.42,0.94,0.94,0.94s0.94-0.42,0.94-0.94
		                C52.8,26.55,52.37,26.13,51.86,26.13z"></path></svg>
                  </div>
                  <p classname='fadeInLeft'>{t("Over 150,000+ client  all over the world.")}</p>
                </div>
              </div>
              <div className="col-lg-6  fadeInLeft order-lg-first ">
                <div className="text-wrapper ">
                  <p className=" display-1 fade-in-image fw-bold">
                  {t("Event ticket organizer & seller.")}
                  </p>
                  <p className='fs-3 fade-in-image py-3'>{t("For hassle-free events, we are here to help you by creating online tickets.")}
                  </p>
                  <button className="theme-btn-five my-2" type="submit">{t("Let’s get started")}</button>
                </div>
              </div>
            </div>
          </div>
          <div className="fancy-feature-eight  mt-5 pt-2">
            <div className="container  px-4">
              <div className="bg-wrapper">
                <div className="row justify-content-center">
                  <div className="col-lg-4 col-md-6 " data-aos="fade-up" data-aos-duration="1200">
                    <div className="block-style-thirteen">
                      <div className="card-body py-auto px-3">
                        <svg height={30} xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-battery" viewBox="0 0 16 16">
                          <path d="M0 6a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v4a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V6zm2-1a1 1 0 0 0-1 1v4a1 1 0 0 0 1 1h10a1 1 0 0 0 1-1V6a1 1 0 0 0-1-1H2zm14 3a1.5 1.5 0 0 1-1.5 1.5v-3A1.5 1.5 0 0 1 16 8z" />
                        </svg> <h5 className="title font-rubik fs-4 fw-bold">{t("Smart popups")}</h5>
                        <p className=" fs-5">{t("Create customized popups and show the message at the lorem")} 
                        </p>

                      </div>
                    </div>
                  </div>
                  <div className="col-lg-4 col-md-6 " data-aos="fade-up" data-aos-duration="1200">
                    <div className="block-style-thirteen">
                      <div className="card-body py-auto px-3">
                        <svg xmlns="http://www.w3.org/2000/svg" height={30} fill="currentColor" className="bi bi-brightness-high" viewBox="0 0 16 16">
                          <path d="M8 11a3 3 0 1 1 0-6 3 3 0 0 1 0 6zm0 1a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM8 0a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0v-2A.5.5 0 0 1 8 0zm0 13a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0v-2A.5.5 0 0 1 8 13zm8-5a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1 0-1h2a.5.5 0 0 1 .5.5zM3 8a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1 0-1h2A.5.5 0 0 1 3 8zm10.657-5.657a.5.5 0 0 1 0 .707l-1.414 1.415a.5.5 0 1 1-.707-.708l1.414-1.414a.5.5 0 0 1 .707 0zm-9.193 9.193a.5.5 0 0 1 0 .707L3.05 13.657a.5.5 0 0 1-.707-.707l1.414-1.414a.5.5 0 0 1 .707 0zm9.193 2.121a.5.5 0 0 1-.707 0l-1.414-1.414a.5.5 0 0 1 .707-.707l1.414 1.414a.5.5 0 0 1 0 .707zM4.464 4.465a.5.5 0 0 1-.707 0L2.343 3.05a.5.5 0 1 1 .707-.707l1.414 1.414a.5.5 0 0 1 0 .708z" />
                        </svg><h5 className="title font-rubik fs-4 fw-bold">{t("Autoresponder")} </h5>
                        <p className=" fs-5">{t("Send welcome email to your new subscribers with a code.")} </p>

                      </div>
                    </div>
                  </div>
                  <div className="col-lg-4 col-md-6 " data-aos="fade-up" data-aos-duration="1200">
                    <div className="block-style-thirteen">
                      <div className="card-body py-auto px-3">
                        <svg xmlns="http://www.w3.org/2000/svg" height="30" fill="currentColor" className="bi bi-coin" viewBox="0 0 16 16">
                          <path d="M5.5 9.511c.076.954.83 1.697 2.182 1.785V12h.6v-.709c1.4-.098 2.218-.846 2.218-1.932 0-.987-.626-1.496-1.745-1.76l-.473-.112V5.57c.6.068.982.396 1.074.85h1.052c-.076-.919-.864-1.638-2.126-1.716V4h-.6v.719c-1.195.117-2.01.836-2.01 1.853 0 .9.606 1.472 1.613 1.707l.397.098v2.034c-.615-.093-1.022-.43-1.114-.9H5.5zm2.177-2.166c-.59-.137-.91-.416-.91-.836 0-.47.345-.822.915-.925v1.76h-.005zm.692 1.193c.717.166 1.048.435 1.048.91 0 .542-.412.914-1.135.982V8.518l.087.02z" />
                          <path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z" />
                          <path d="M8 13.5a5.5 5.5 0 1 1 0-11 5.5 5.5 0 0 1 0 11zm0 .5A6 6 0 1 0 8 2a6 6 0 0 0 0 12z" />
                        </svg>
                        <h5 className="title font-rubik fs-4 fw-bold">{t("Embeded Forms")}</h5>
                        <p className=" fs-5"> {t("Collect website leads with embedded forms and integrate easily.")}</p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className='container px-5 h-100 my-5 pt-5' >
          <div className='row justify-content-center fadeInLeft'>
            <div className='card h-100 col-md-6 order-md-1 px-0 border-0' data-aos="fade-right" data-aos-duration="1200">
              <div className="  text-dark">
                <Image src={s1} className="card-img border-0 image-fluid" alt="..." />
              </div>
            </div>

            <div className='col-md-6 order-md-2 text-left  px-0' data-aos="fade-left" data-aos-duration="1200">
              <div className=' rounded-0 h-100  proh position-relative overflow-hidden  d-flex justify-content-center flex-column h-100'>
                <div className="container ff card fs-5   lh-lg proh mx-auto border-0 ">
                  <p classname=''><span>{t("Over 150,000+ client  all over the world.")}</span></p>

                  <p className=" display-3 fw-bold ">{t('Best event & online ticket platform.')}
                  </p>
                  <p className="fs-5  ">{t('Things go wrong. You’ll have questions. We understand. So we have people, not bots, on hand to help.')}
                  </p>
                  <p className="fs-5 ">{t('We aim to answer any query in less than 10 minutes.')}
                  </p>
                  <p className="fs-5 fw-bold ">{t("-Mitchell Garner")}
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="fancy-feature-nine ">
          <div className="container ">
            <div className="title-style-five text-center 0">
              <h6>{t("GET STARTED IN MINUTES")}</h6>
              <h2><span>{t("3 simple & easy step to launch.")}</span></h2>
            </div>
            <div className="row justify-content-center text-center">
              <div className="col-lg-4 col-md-6 " data-aos="fade-up" data-aos-duration="1200">
                <div className="block-style-fourteen">
                  <div className="illustration">
                    <Image src={s2} height={225} alt="icon" />
                  </div>
                  <div className="title">{t("Join Our Platform")}</div>
                  <p className="font-rubik">{t("It only takes 5 minutes. Set up is smooth and simple fully lorem qui.")}  </p>
                </div>
              </div>
              <div className="col-lg-4 col-md-6" data-aos="fade-up" data-aos-duration="1200">
                <div className="block-style-fourteen">
                  <div className="illustration">
                    <Image src={s3} height={225} alt="icon" />
                  </div>
                  <div className="title">{t("Manage you event")}</div>
                  <p className="font-rubik">{t("Choose a price package to suit you or pay per ticket. Sell on yourwebsite,")} </p>
                </div>
              </div>
              <div className="col-lg-4 col-md-6 " data-aos="fade-up" data-aos-duration="1200">
                <div className="block-style-fourteen">
                  <div className="illustration">
                    <Image src={s4} height={225} alt="icon" />
                  </div>
                  <div className="title">{t("Start selling ticket")}</div>
                  <p className="font-rubik">{t("Get payments, detailed data on tickets sales & message guests lorem")}  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="fancy-text-block-six overflow-hidden my-5">
          <div className="container px-2  align-self-center">
            <div className="row align-items-center justify-content-center  align-self-center">
              <div className="col-lg-6 " data-aos="fade-right" data-aos-duration="1200">
                <div className="title-style-five mb-35">
                  <div className="title-style-five mb-35">
                    <h6>{t("Why choose us")}
                    </h6><h2>
                      <span>{t("Why you should")}
                      </span>{t("choose us?")} 
                    </h2>
                  </div>
                  <div className="accordion-style-two pe-5 pt-3">
                    <div className="faq-wrraper">
                      <div className="accordion" id="accordionExample">
                        <div className=" card p-4">
                          <h5 className="accordion-header mb-2 " id="headingOne">
                            <button className="card-header fw-bold fs-5 text-black accordion-button" type="button"
                              style={{ boxShadow: "none" }} data-bs-toggle="collapse" data-bs-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                             {t("Register and create your first.")} 
                            </button>
                          </h5>
                          <div id="collapseOne" className="accordion-collapse collapse show" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <h6 className="card-body  fs-6 pt-2  p-0">
                             {t("It only takes 5 minutes. Set-up is smooth & simple, with fully customisable page design to reflect your brand lorem dummy.")}  </h6>
                          </div>
                        </div>
                        <div className="card p-4">
                          <h5 className="accordion-header mb-2" id="headingTwo">
                            <button className="card-header fw-bold fs-5 text-black accordion-button" type="button"
                              style={{ boxShadow: "none" }} data-bs-toggle="collapse" data-bs-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                              {t("Manage your client easily")} 
                            </button>
                          </h5>
                          <div id="collapseTwo" className="accordion-collapse collapse" aria-labelledby="headingTwo" data-bs-parent="#accordionExample">
                            <h6 className="card-body  fs-6 pt-2 p-0">
                            {t("It only takes 5 minutes. Set-up is smooth & simple, with fully customisable page design to reflect your brand lorem dummy.")} 
                          </h6>
                          </div>
                        </div>
                        <div className="card p-4">
                          <h5 className="accordion-header mb-2" id="headingThree">
                            <button className="card-header fw-bold fs-5 text-black accordion-button" type="button"
                              style={{ boxShadow: "none" }} data-bs-toggle="collapse" data-bs-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
                               {t("Start selling ticket")} 
                            </button>
                          </h5>
                          <div id="collapseThree" className="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#accordionExample">
                            <h6 className="card-body fs-6 pt-2 p-0">
                            {t("It only takes 5 minutes. Set-up is smooth & simple, with fully customisable page design to reflect your brand lorem dummy.")} 
                         </h6>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-lg-6 col-md-8  justify-content-center  " data-aos="fade-left"
                data-aos-duration="1200">
                <div className="illustration-holder  align-items-center d-flex justify-content-center ">
                  <Image src={s5} alt="media" />
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="pricing-section-three my-5">
          <div className="container">
            <div className="title-style-five text-center ff  mb-50 md-mb-40">
              <h6 >
              {t("Our Pricing")} 
              </h6>
              <div className="row">
                <div className="col-lg-9 m-auto ">
                  <h2 className='ff'>{t("Unbeatable prices, no contracts, simple & easy")} 
                  </h2>
                </div>
                <div className="pricing-table-area-three">
                  <div className="row justify-content-center">
                    <div className="col-lg-4 col-md-6 " data-aos="fade-up" data-aos-duration="1200"
                      data-aos-delay="0">
                      <div className="pr-table-wrapper skew-right">
                        <div className="pack-name ff fs-4">
                          <span> {t("Free Event")} </span>
                        </div>
                        <div className="price font-slab">$0</div>
                        <p className="user-condition">{t("per user/month")}</p>

                        <ul><li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                          <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                        </svg>{t("60-day chat history")}</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Basic widget customization")}</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Ticketing system")}</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Data security")}</li>
                        </ul>
                        <button className="theme-btn-five my-2 " href="#" >Subscribe Now</button>
                      </div>
                    </div>
                    <div className="col-lg-4 col-md-6 " data-aos="fade-up"
                      data-aos-duration="1200" data-aos-delay="100">
                      <div className="pr-table-wrapper skew-left">
                        <div className="pack-name ff fs-4">
                          <span>{t("Pre Day")}</span>
                        </div>
                        <div className="price font-slab">$0.32</div>
                        <p className="user-condition">{t("per user/month")}</p>

                        <ul>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("60-day chat history")}</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Basic widget customization")}</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Ticketing system")}</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Data security")}</li>
                        </ul>
                        <button className="theme-btn-five my-2 " href="#" >Subscribe Now</button>
                      </div>
                    </div>
                    <div className="col-lg-4 col-md-6 " data-aos="fade-up"
                      data-aos-duration="1200" data-aos-delay="200">
                      <div className="pr-table-wrapper skew-right">
                        <div className="pack-name ff fs-4">
                          <span>{t("Pay As You Go")}</span>
                        </div>
                        <div className="price font-slab">$0.65</div>
                        <p className="user-condition">{t("per user/month")}</p>

                        <ul>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>60-day chat history</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Basic widget customization")}</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Ticketing system")}</li>
                          <li className=""><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-check" viewBox="0 0 16 16">
                            <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.267.267 0 0 1 .02-.022z" />
                          </svg>{t("Data security")}</li>
                        </ul>
                        <button className="theme-btn-five my-2 " href="#" >Subscribe Now</button>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div className="client-feedback-slider-three mt-5 pt-5">
          <div className="container my-4 px-5">
            <div className="row">
              <div className="col-xl-7 col-lg-9 m-auto"><div className="testimonial-slider-wrapper">
                <div className="clientSliderThree">
                  <div data-index="0" className="slick-slide pb-5 slick-active slick-current" tabindex="-1"
                    aria-hidden="false" >
                    <div>
                      <div className="title-style-five text-center mb-80 md-mb-50">
                        <div className="col-lg-7 col-md-9 m-auto mt-5">
                          <h2 data-aos="fade-up" data-aos-duration="1200" className="">
                            <span>{t("Check what’s our client")}
                            </span>
                            {t("say about us.")} 
                          </h2>
                        </div>
                      </div>
                      <div className="mt-4 ">
                        <svg xmlns="http://www.w3.org/2000/svg" height="77" fill="currentColor" className="bi bi-chat-square-quote-fill" viewBox="0 0 16 16">
                          <path d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-2.5a1 1 0 0 0-.8.4l-1.9 2.533a1 1 0 0 1-1.6 0L5.3 12.4a1 1 0 0 0-.8-.4H2a2 2 0 0 1-2-2V2zm7.194 2.766a1.688 1.688 0 0 0-.227-.272 1.467 1.467 0 0 0-.469-.324l-.008-.004A1.785 1.785 0 0 0 5.734 4C4.776 4 4 4.746 4 5.667c0 .92.776 1.666 1.734 1.666.343 0 .662-.095.931-.26-.137.389-.39.804-.81 1.22a.405.405 0 0 0 .011.59c.173.16.447.155.614-.01 1.334-1.329 1.37-2.758.941-3.706a2.461 2.461 0 0 0-.227-.4zM11 7.073c-.136.389-.39.804-.81 1.22a.405.405 0 0 0 .012.59c.172.16.446.155.613-.01 1.334-1.329 1.37-2.758.942-3.706a2.466 2.466 0 0 0-.228-.4 1.686 1.686 0 0 0-.227-.273 1.466 1.466 0 0 0-.469-.324l-.008-.004A1.785 1.785 0 0 0 10.07 4c-.957 0-1.734.746-1.734 1.667 0 .92.777 1.666 1.734 1.666.343 0 .662-.095.931-.26z" />
                        </svg></div>
                      <div className="item " data-aos="fade-up" data-aos-duration="1200"
                        data-aos-delay="100" tabindex="-1" >

                        <p>{t(" Having a home based business is a wonderful asset to your life. The problem still stands it comes time advertise your business for a cheap cost. I know you have looked answer everywhere.")} 
                        </p>
                        <h6 className="name"> {t("Jannatul Ekra")} 
                        </h6>
                        <span className="desig">{t("Designer")} 
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className='' data-aos="fade-up" data-aos-duration="1200">
                    <svg xmlns="http://www.w3.org/2000/svg" height="100" fill="currentColor" className="bi bi-dash-lg" viewBox="0 0 16 16">
                      <path fill-rule="evenodd" d="M2 8a.5.5 0 0 1 .5-.5h11a.5.5 0 0 1 0 1h-11A.5.5 0 0 1 2 8Z" />
                    </svg>
                  </div>
                  <div data-index="1" className="slick-slide pb-5" tabindex="-1" aria-hidden="true"
                  >  <div className="title-style-five text-center mb-80 md-mb-50">
                      <div className="col-lg-7 col-md-9 m-auto mt-5">
                        <h2 data-aos="fade-up" data-aos-duration="1200" className="">
                        <span>{t("Check what’s our client")}
                            </span>
                            {t("say about us.")} 
                          </h2>
                      </div>
                    </div>
                    <div className="mt-4 " data-aos="fade-up" data-aos-duration="1200">
                      <svg xmlns="http://www.w3.org/2000/svg" height="77" fill="currentColor" className="bi bi-chat-square-quote-fill" viewBox="0 0 16 16">
                        <path d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2h-2.5a1 1 0 0 0-.8.4l-1.9 2.533a1 1 0 0 1-1.6 0L5.3 12.4a1 1 0 0 0-.8-.4H2a2 2 0 0 1-2-2V2zm7.194 2.766a1.688 1.688 0 0 0-.227-.272 1.467 1.467 0 0 0-.469-.324l-.008-.004A1.785 1.785 0 0 0 5.734 4C4.776 4 4 4.746 4 5.667c0 .92.776 1.666 1.734 1.666.343 0 .662-.095.931-.26-.137.389-.39.804-.81 1.22a.405.405 0 0 0 .011.59c.173.16.447.155.614-.01 1.334-1.329 1.37-2.758.941-3.706a2.461 2.461 0 0 0-.227-.4zM11 7.073c-.136.389-.39.804-.81 1.22a.405.405 0 0 0 .012.59c.172.16.446.155.613-.01 1.334-1.329 1.37-2.758.942-3.706a2.466 2.466 0 0 0-.228-.4 1.686 1.686 0 0 0-.227-.273 1.466 1.466 0 0 0-.469-.324l-.008-.004A1.785 1.785 0 0 0 10.07 4c-.957 0-1.734.746-1.734 1.667 0 .92.777 1.666 1.734 1.666.343 0 .662-.095.931-.26z" />
                      </svg></div>
                    <div>
                      <div className="item " data-aos="fade-up" data-aos-duration="1200"
                        data-aos-delay="100" tabindex="-1" >
                       <p>{t(" Having a home based business is a wonderful asset to your life. The problem still stands it comes time advertise your business for a cheap cost. I know you have looked answer everywhere.")} 
                        </p>
                        <h6 className="name">{t("Bostami Hasan ")} 
                        </h6>
                        <span className="desig">{t("Front-end Engineer")} 
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              </div>
            </div>
          </div>
        </div>

        <div className="faq-section-four">
          <div className="container">
            <div className="container">
              <div className="title-style-five text-center mb-80 md-mb-60">
                <h6>{t("FAQ")} 
                </h6>
                <h2>
                  <span>{t("Question & Answer")} 
                  </span>
                </h2>
              </div>
              <div className="row">
                <div className="col-xl-9 col-lg-10 m-auto">
                  <div className="accordion-style-four " data-aos="fade-up" data-aos-duration="1200">
                    <div className="faq-wrraper p-3">
                      <div data-accordion-component="Accordion" id="accordionExample" className="accordion  ">
                        {/* <div data-accordion-component="AccordionItem" className="card"> */}
                        <div data-accordion-component="AccordionItem " className="bg-white mb-4 py-0">
                          <div data-accordion-component="AccordionItemHeading" className="card-header " role="heading" aria-level="3">
                            <div className=" fw-bold fs-2 md-5 text-black hhbt   accordion-button" id="accordion__heading-a" data-bs-toggle="collapse" data-bs-target="#accordion__panel-a" aria-expanded="true"
                              style={{ boxShadow: "none" }} aria-disabled="false" aria-controls="accordion__panel-a" role="button" tabindex="0" data-accordion-component="AccordionItemButton">

                              
                              {t("How does the free trial work?")} 
                            </div>
                          </div>
                          <div id="accordion__panel-a" className="card-body collapse show fadeInUp p-3" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <p className=' fs-5'> {t("mea case duis tollit et. Etiam nusquam set minium eu sea, ei tale paulo elab. Noluisse mnesarch Et is vero incorrupte eos deserunt quaeren.")} </p>
                          </div>

                        </div>

                        <div data-accordion-component="AccordionItem" className="bg-white mb-4">
                          <div data-accordion-component="AccordionItemHeading" className="card-header" role="heading" aria-level="3">
                            <div className="fw-bold fs-2 md-5 text-black hhbt  accordion-button" id="accordion__heading-a" data-bs-toggle="collapse" data-bs-target="#accordion__panel-b" aria-expanded="true"
                              style={{ boxShadow: "none" }} aria-disabled="false" aria-controls="accordion__panel-b" role="button" tabindex="0" data-accordion-component="AccordionItemButton">
                                {t("How do you weigh different criteria in your process?")} 
                            </div>
                          </div>
                          <div id="accordion__panel-b" className="card-body collapse fadeInUp p-3" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <p className='fs-5'>{t("mea case duis tollit et. Etiam nusquam set minium eu sea, ei tale paulo elab. Noluisse mnesarch Et is vero incorrupte eos deserunt quaeren.")} </p>
                          </div>
                        </div>
                        <div data-accordion-component="AccordionItem" className="bg-white mb-4">
                          <div data-accordion-component="AccordionItemHeading" className="card-header" role="heading" aria-level="3">
                            <div className="fw-bold fs-2 md-5 text-black hhbt  accordion-button" id="accordion__heading-a" data-bs-toggle="collapse" data-bs-target="#accordion__panel-c" aria-expanded="true"
                              style={{ boxShadow: "none" }} aria-disabled="false" aria-controls="accordion__panel-c" role="button" tabindex="0" data-accordion-component="AccordionItemButton">
                              {t("What does First Round look for in an idea?")} 
                            </div>
                          </div>
                          <div id="accordion__panel-c" className="card-body collapse fadeInUp p-3" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <p className='fs-5'>{t("mea case duis tollit et. Etiam nusquam set minium eu sea, ei tale paulo elab. Noluisse mnesarch Et is vero incorrupte eos deserunt quaeren.")} </p>
                          </div>
                        </div>
                        <div data-accordion-component="AccordionItem" className="bg-white mb-4">
                          <div data-accordion-component="AccordionItemHeading" className="card-header" role="heading" aria-level="3">
                            <div className="fw-bold fs-2 md-5 text-black hhbt c  accordion-button" id="accordion__heading-a" data-bs-toggle="collapse" data-bs-target="#accordion__panel-d" aria-expanded="true"
                              style={{ boxShadow: "none" }} aria-disabled="false" aria-controls="accordion__panel-d" role="button" tabindex="0" data-accordion-component="AccordionItemButton">
                              {t("What do you look for in a founding team?")} 
                            </div>
                          </div>
                          <div id="accordion__panel-d" className="card-body collapse fadeInUp p-3" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <p className='fs-5'>{t("mea case duis tollit et. Etiam nusquam set minium eu sea, ei tale paulo elab. Noluisse mnesarch Et is vero incorrupte eos deserunt quaeren.")} </p>
                          </div>
                        </div>
                        <div data-accordion-component="AccordionItem" className="bg-white mb-4">
                          <div data-accordion-component="AccordionItemHeading" className="card-header" role="heading" aria-level="3">
                            <div className="fw-bold fs-2 md-5 text-black hhbt  accordion-button" id="accordion__heading-a" data-bs-toggle="collapse" data-bs-target="#accordion__panel-e" aria-expanded="true"
                              style={{ boxShadow: "none" }} aria-disabled="false" aria-controls="accordion__panel-e" role="button" tabindex="0" data-accordion-component="AccordionItemButton">
                                {t(" Do you recommend Pay as you go or Pre pay?")} 
                            </div>
                          </div>
                          <div id="accordion__panel-e" className="card-body collapse fadeInUp p-3" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                            <p className='fs-5'>{t("mea case duis tollit et. Etiam nusquam set minium eu sea, ei tale paulo elab. Noluisse mnesarch Et is vero incorrupte eos deserunt quaeren.")} </p>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="text-center m-4  " data-aos="fade-up" data-aos-duration="1200">
                <h3 className="font-rubik pb-3">{t("Don’t find your answer?")} 
               </h3>
                <a className="theme-btn-five p-3 fw" href="/contact-eo">Contact us</a>
              </div>
            </div>
          </div>
        </div>

        <div className="fancy-short-banner-five px-5 py-5">
          <div className="container">
            <div className="title-style-five text-center">
              <h2>
                <span>{t("Sign up & get started")} </span>
              </h2>
            </div>
            <div className="row">
              <div className="col-xl-7 col-lg-8 col-md-9 m-auto">
                <p className="font-rubik sub-heading">{t("Our ticketing platform is made for you - & loved by everyone lorem ipsum.")}</p>
              </div>
            </div>
            <div className="row">
              <div className="col-xl-7 col-lg-8 col-md-10 m-auto">
                <form>
                  <input type="text" placeholder="Email address" />
                  <button className='theme-btn-five fw'>{t("Start Trial")}</button>
                </form>
                <p className="info-text">{t("Already a member?")}
                  <a href="/login">{t("Sign in.")}</a>
                </p>
              </div>
            </div>
          </div>
        </div>

        <footer className="theme-footer-four ff ">
          <div className="top-footer">
            <div className="container">
              <div className="row justify-content-end">
                <div className="col-xl-4 col-lg-3 col-12 footer-about-widget ">
                  <Link className='pb-4' href="/">
                    <svg baseProfile="tiny" height={20} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 304 36"><path fill="#81B441" d="M22.038 6.715c-.865-.48-3.344-.183-6.322.738-5.212 3.562-9.61 8.811-9.917 17.238-.056.202-.57-.027-.672-.09-1.407-2.697-1.965-5.538-.79-9.635.219-.365-.497-.814-.626-.685-.258.258-1.333 1.404-2.047 2.642-3.54 6.141-1.225 14.009 4.968 17.453 6.192 3.449 14.011 1.224 17.459-4.97 3.989-7.148.285-21.385-2.053-22.691z" /><g fill="#fff"><path d="M44.202 10.145c-7.342 0-12.671 5.458-12.671 12.978 0 7.582 5.315 12.877 12.927 12.877 3.845 0 6.888-1.202 9.309-3.677.723-.781.801-1.533.801-1.821 0-1.226-.912-2.117-2.168-2.117-.633 0-1.163.239-1.77.801-1.371 1.277-3.176 2.582-6.172 2.582-4.166 0-7.554-3.128-7.768-7.145h16.581c1.803 0 2.679-.843 2.679-2.576 0-.51 0-.879-.103-1.605-.893-6.447-5.246-10.297-11.645-10.297zm0 4.026c3.981 0 6.536 2.537 6.693 6.632h-14.2c.373-3.789 3.577-6.632 7.507-6.632zM71.785 10.145c-3.335 0-6.637 1.918-7.927 4.581v-1.646c0-2.55-1.976-2.679-2.372-2.679-1.171 0-2.423.704-2.423 2.679v19.881c0 2.648 2.104 2.782 2.525 2.782.421 0 2.526-.134 2.526-2.782v-10.657c0-4.637 2.628-7.876 6.39-7.876 3.644 0 5.416 2.391 5.416 7.312v11.222c0 2.648 2.105 2.782 2.526 2.782.421 0 2.525-.134 2.525-2.782v-12.81c0-4.838-2.414-10.007-9.186-10.007zM103.825 10.402c-1.167 0-2.014.681-2.518 2.02l-6.911 17.378-6.858-17.378c-.537-1.36-1.377-2.02-2.569-2.02-1.393 0-2.526 1.087-2.526 2.423 0 .331.038.759.317 1.429l7.942 18.753c.946 2.262 2.503 2.737 3.643 2.737 1.139 0 2.696-.475 3.645-2.736l7.994-18.857c.273-.662.315-1.147.315-1.377 0-1.352-1.065-2.372-2.474-2.372zM118.513 10.145c-3.231 0-6.24.9-8.472 2.532-.757.522-1.125 1.174-1.125 1.993 0 1.128.861 2.012 1.963 2.012.476 0 1.01-.19 1.499-.532 1.919-1.279 3.722-1.876 5.673-1.876 3.735 0 5.879 1.899 5.879 5.212v.549c-8.384.009-16.91 1.022-16.91 8.315 0 5.249 4.486 7.599 8.932 7.599 3.54 0 6.368-1.403 8.182-4.059v1.277c0 1.902 1.277 2.576 2.373 2.576.231 0 2.269-.072 2.269-2.576v-13.373c-.002-6.042-3.839-9.649-10.263-9.649zm4.354 13.609h1.063v1.164c0 4.256-2.71 7.006-6.903 7.006-1.138 0-4.852-.265-4.852-3.676-.001-4.029 6.101-4.494 10.692-4.494zM141.928 14.991c2.21 0 2.322-1.72 2.322-2.064 0-1.023-.61-2.116-2.322-2.116h-4.545v-4.596c0-2.053-1.332-2.782-2.474-2.782-.421 0-2.525.134-2.525 2.782v21.829c0 5.037 2.485 7.7 7.187 7.7 1.217 0 2.402-.215 3.178-.576.889-.444 1.398-1.173 1.398-2 0-1.167-.847-2.014-2.014-2.014-.225 0-.546.056-.9.158-.433.108-.726.15-1.046.15-1.913 0-2.804-1.249-2.804-3.931v-12.54h4.545zM158.631 10.145c-7.64 0-13.183 5.415-13.183 12.875 0 3.632 1.343 6.942 3.78 9.321 2.419 2.359 5.758 3.659 9.402 3.659 7.544 0 13.234-5.58 13.234-12.979.001-7.461-5.566-12.876-13.233-12.876zm0 21.573c-5.239 0-7.979-4.375-7.979-8.697 0-5.939 4.136-8.644 7.979-8.644s7.979 2.705 7.979 8.644c0 5.975-4.136 8.697-7.979 8.697z" /></g><g fill="#81B441"><path d="M211.554 11.937c-.739-.805-1.655-1.403-2.725-1.777-1.951-.68-4.619-.735-6.739-.061-.994.316-1.909.754-2.722 1.302-.815.552-1.53 1.207-2.126 1.947-.187.231-.363.47-.527.715-.52-1.284-1.285-2.283-2.288-2.978-1.401-.972-3.205-1.465-5.363-1.465-1.102 0-2.146.176-3.105.522-.947.342-1.823.815-2.604 1.404-.767.579-1.448 1.273-2.023 2.061v-2.787c0-.418-.339-.757-.757-.757h-1.219c-.418 0-.757.339-.757.757v23.98c0 .418.339.757.757.757h1.219c.418 0 .757-.339.757-.757v-15.087c.14-.709.395-1.502.759-2.359.368-.865.874-1.691 1.504-2.451.617-.744 1.387-1.373 2.288-1.871.872-.481 1.92-.725 3.114-.725.873 0 1.69.102 2.426.305.689.19 1.266.511 1.763.982.493.469.886 1.122 1.169 1.94.296.854.446 1.967.446 3.309v15.957c0 .418.339.757.757.757h1.197c.418 0 .757-.339.757-.757l-.002-15.906c.025-.361.117-.778.272-1.238.16-.473.387-.957.675-1.44.292-.49.654-.975 1.077-1.445.41-.457.902-.872 1.464-1.233.559-.36 1.203-.655 1.911-.879.705-.221 1.502-.339 2.368-.353l.155-.001c.806 0 1.562.101 2.247.303.688.202 1.264.541 1.761 1.04.496.495.892 1.196 1.178 2.08.299.925.451 2.121.451 3.557v15.515c0 .418.339.757.757.757h1.197c.418 0 .757-.339.757-.757v-15.515c0-1.664-.187-3.111-.555-4.302-.377-1.224-.94-2.249-1.671-3.046zM235.82 33.56c-.07-.337-.129-.697-.178-1.083-.05-.392-.089-.804-.117-1.239-.029-.434-.043-.857-.043-1.27v-12.012c0-1.355-.234-2.573-.697-3.62-.467-1.055-1.129-1.945-1.969-2.646-.828-.689-1.823-1.215-2.957-1.561-2.266-.689-5.181-.69-7.45.098-1.161.401-2.186.958-3.045 1.656-.869.708-1.554 1.542-2.035 2.481-.489.953-.737 1.968-.737 3.015 0 .407.322.741.729.756l1.197.044c.202.016.404-.069.553-.211.148-.142.232-.339.232-.545 0-.705.161-1.344.494-1.953.339-.62.814-1.166 1.413-1.623.615-.47 1.354-.843 2.197-1.111.852-.27 1.797-.407 2.807-.407 2.149 0 3.805.496 4.92 1.474 1.101.967 1.637 2.31 1.637 4.108v2.745h-4.984c-1.783 0-3.413.191-4.847.568-1.45.383-2.701.925-3.721 1.612-1.049.708-1.863 1.575-2.421 2.575-.564 1.011-.85 2.127-.85 3.316 0 1.061.197 2.05.584 2.939.389.895.953 1.676 1.674 2.323.716.642 1.585 1.143 2.583 1.491.986.344 2.095.519 3.297.519 1.255 0 2.417-.139 3.451-.414 1.031-.272 1.979-.651 2.817-1.125.839-.475 1.585-1.033 2.22-1.66l.275-.281.024.315c.071.837.195 1.568.371 2.175.093.324.39.547.727.547h1.374c.418 0 .757-.339.757-.757v-.199c0-.104-.022-.208-.064-.303-.081-.187-.155-.435-.218-.737zm-11.734-.248c-.825 0-1.58-.122-2.247-.365-.66-.239-1.235-.571-1.708-.987-.466-.409-.833-.891-1.091-1.43-.253-.531-.38-1.107-.38-1.714 0-.768.189-1.455.578-2.102.396-.658.978-1.234 1.731-1.715.783-.499 1.758-.895 2.9-1.177 1.163-.287 2.503-.433 3.985-.433h4.917v4.774c-.357.761-.799 1.452-1.315 2.058-.546.641-1.189 1.198-1.913 1.658-.722.459-1.547.816-2.452 1.061-.913.246-1.924.372-3.005.372zM250.599 9.913c-.244-.094-.527-.166-.863-.218-.319-.051-.687-.076-1.093-.076-2.123 0-3.924.544-5.352 1.615-.644.482-1.216 1.046-1.715 1.688v-2.102c0-.418-.339-.757-.757-.757h-1.197c-.418 0-.757.339-.757.757v23.98c0 .418.339.757.757.757h1.197c.418 0 .757-.339.757-.757v-15.646c.18-.986.466-1.908.851-2.744.383-.833.875-1.564 1.463-2.172.579-.598 1.273-1.074 2.062-1.416.787-.341 1.692-.514 2.691-.514.251 0 .492.013.723.039l.735.088c.198.024.4-.031.558-.153.158-.123.261-.304.287-.503l.133-1.064c.042-.347-.155-.677-.48-.802zM260.642 21.099l10.484-9.725c.228-.212.303-.541.19-.832-.114-.289-.393-.48-.704-.48h-1.707c-.19 0-.374.072-.513.202l-10.911 10.09-1.538 1.454v-21.051c0-.418-.339-.757-.757-.757h-1.197c-.418 0-.757.339-.757.757v34.043c0 .418.339.757.757.757h1.197c.418 0 .757-.339.757-.757v-9.582l2.687-2.31 10.541 12.383c.144.169.355.266.576.266h1.573c.418 0 .757-.339.757-.757 0-.244-.116-.461-.295-.599l-11.14-13.102zM288.498 12.708c-.873-.976-1.929-1.745-3.139-2.281-2.474-1.098-5.643-1.093-8.216.154-1.307.633-2.462 1.528-3.435 2.66-.964 1.121-1.738 2.475-2.299 4.024-.559 1.547-.843 3.256-.843 5.079v.975c0 1.827.292 3.531.869 5.064.577 1.539 1.384 2.888 2.396 4.008 1.016 1.127 2.22 2.019 3.578 2.651 1.366.635 2.853.957 4.42.957 1.015 0 1.963-.104 2.815-.309.853-.206 1.645-.495 2.353-.861.713-.369 1.361-.818 1.925-1.333.557-.509 1.056-1.081 1.486-1.701.235-.341.154-.807-.184-1.047l-.842-.598c-.33-.236-.788-.169-1.036.154-.462.597-.945 1.091-1.437 1.47-.491.379-1.009.686-1.54.911-.532.227-1.096.388-1.675.481-1.725.278-3.632.16-5.208-.618-1.041-.514-1.957-1.223-2.72-2.109-.77-.893-1.384-1.961-1.823-3.177-.441-1.219-.664-2.547-.664-3.943v-.551h17.218c.418 0 .757-.339.757-.757v-1.396c0-1.602-.243-3.09-.721-4.422-.485-1.342-1.169-2.514-2.035-3.485zm-4.19.263c.876.432 1.632 1.024 2.248 1.757.625.747 1.117 1.63 1.461 2.623.298.858.47 1.767.512 2.707h-15.079c.155-.911.406-1.775.75-2.58.444-1.037 1.027-1.954 1.734-2.722.699-.759 1.518-1.36 2.435-1.789 1.795-.838 4.188-.855 5.939.004zM303.675 33.715c-.071-.401-.446-.661-.852-.617-.403.058-.778.106-1.124.141-.654.069-1.575.107-2.316-.108-.303-.087-.566-.255-.806-.518-.251-.272-.459-.683-.617-1.219-.17-.577-.257-1.363-.257-2.335v-16.264h4.696c.418 0 .757-.339.757-.757v-1.219c0-.418-.339-.757-.757-.757h-4.696v-5.715c0-.418-.339-.757-.757-.757h-1.219c-.418 0-.757.339-.757.757v5.715h-3.676c-.418 0-.757.339-.757.757v1.219c0 .418.339.757.757.757h3.676v16.265c0 1.26.121 2.335.36 3.195.252.907.631 1.649 1.128 2.207.511.573 1.142.983 1.877 1.215.677.215 1.448.324 2.29.324l.583-.012c.234-.008.475-.024.732-.047l.713-.08c.275-.037.522-.095.735-.172.345-.126.55-.481.487-.843l-.2-1.132z" /></g>
                    </svg>
                  </Link>
                  <ul className="ff ">
                    <li>
                      <a href="mailto:deskiinc@gmail.com">deskiinc@gmail.com</a>
                    </li>
                    <li>
                      <a href="tel:+761 412 3224">+761 412 3224</a>
                    </li>
                  </ul>
                </div>
                <div className="col-lg-3 col-md-4 footer-list ff">
                  <h5 className="footer-title  ff">Links</h5>
                  <ul>
                    <li>
                      <a href="/" className='ff'>Home</a>
                    </li>
                    <li>
                      <a href="/product-customer-support" className='ff'>Product</a>
                    </li>
                    <li>
                      <a href="/pricing-pm" className='ff'>Pricing</a>
                    </li>
                    <li>
                      <a href="/about-pm" className='ff'>About us</a>
                    </li>
                    <li>
                      <a href="/solution-management" className='ff'>Features</a>
                    </li>
                    <li>
                      <a href="/blog-v4" className='ff'>Blog</a>
                    </li>
                    <li>
                      <a href="/login" className='ff'>Help &amp; Support</a>
                    </li>
                    <li>
                      <a href="/doc-full-width" className='ff'>API documentation</a>
                    </li>
                  </ul>
                </div>
                <div className="col-lg-3 col-md-4 footer-list">
                  <h5 className="footer-title ff">Legal</h5>
                  <ul>
                    <li>
                      <a href="/terms-conditions" className='ff'>Terms of use</a>
                    </li>
                    <li>
                      <a href="/terms-conditions" className='ff'>Terms &amp; conditions</a>
                    </li>
                    <li>
                      <a href="/terms-conditions" className='ff'>Privacy policy</a>
                    </li>
                    <li>
                      <a href="/">Cookie policy</a>
                    </li>
                  </ul>
                </div>
                <div className="col-xl-2 col-lg-3 col-md-4 footer-list">
                  <h5 className="footer-title  ff">Products</h5>
                  <ul className="pe-5">
                    <li>
                      <a href="/product-customer-support" className='ff'>Take the tour</a>
                    </li>
                    <li>
                      <a href="/team-5" className='ff'>Team</a>
                    </li>
                    <li>
                      <a href="/features-customer-support" className='ff'>Self Service</a>
                    </li>
                    <li>
                      <a href="/blog-v3" className='ff'>News</a>
                    </li>
                    <li>
                      <a href="/about-doc" className='ff'>About us</a>
                    </li>
                    <li>
                      <a href="/contact-cs" className='ff'>Contact</a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <div className="container">
            <div className="bottom-footer-content pt-5 mt-5 p-3">
              <p className='ff fw-bold'>Copyright @2023
                <a href="https://themeforest.net/user/ib-themes/portfolio" className='ff' target="_blank" rel="noreferrer">ib-themes</a> inc.
              </p>
            </div>
          </div>
        </footer>

        <div></div>

        <div></div>


      </main >
    </>
  )
}
//